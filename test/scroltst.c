...
TESTFUNC(scrolltest)
{
    ...
    for (;;) {
        ...
        int y = (vh / 3) - (3 * txh);
        GrxTextOptions *text_opt = grx_text_options_new_full(
            font, txc, bgc, GRX_TEXT_HALIGN_LEFT, GRX_TEXT_VALIGN_TOP);
        grx_clear_screen(bgc);
        drawing(0, 0, vw, vh, fgc, bgc);
        sprintf(buff, l1, wdt, hgt);
        grx_draw_text(buff, x, y, text_opt);
        y += txh;
        sprintf(buff, l2, vw, vh);
        grx_draw_text(buff, x, y, text_opt);
        y += txh;
        for (;; grx_set_viewport(vx, vy)) {
            ...
            sprintf(buff, l3, vx, vy);
            grx_draw_text(buff, x, yy, text_opt);
            yy += txh;
            grx_draw_text(l4, x, yy, text_opt);
            yy += txh;
            grx_draw_text(l5, x, yy, text_opt);
            yy += txh;
            grx_draw_text(l6, x, yy, text_opt);
            yy += txh;
