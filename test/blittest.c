...
TESTFUNC(blittest)
{
    ...
    GrxContext *save =
        grx_context_new_subcontext(0, 0, grx_get_max_x(), grx_get_max_y(), NULL, NULL);
    ...
    icolors.background = bg = grx_color_get(30, 30, 30);
    ...
    for (ii = 0, by = -(bh / 3); ii < 19; ii++) {
        for (jj = 0, bx = -(bw / 2); jj < 19; jj++) {
            grx_draw_framed_box(bx + 2 * wdt, by + 2 * wdt, bx + bw - 2 * wdt - 1,
                by + bh - 2 * wdt - 1, 2 * wdt, &bcolors);
            bx += bw;
        }
        by += bh;
    }

    grx_draw_framed_box(ww / 4 - 5 * wdt - 1, wh / 4 - 5 * wdt - 1,
        ww / 4 + 5 * wdt + ww + 1, wh / 4 + 5 * wdt + wh + 1, wdt, &ocolors);
    grx_draw_framed_box(
        ww / 4 - 1, wh / 4 - 1, ww / 4 + ww + 1, wh / 4 + wh + 1, wdt, &icolors);
    ...
    for (cnt = 0; cnt < 3; cnt++) {
        for (ii = 0, by = -(bh / 3); ii < 19; ii++) {
            for (jj = 0, bx = -(bw / 2); jj < 19; jj++) {
                if ((ii != 15) || (jj != 15)) {
                    grx_context_bit_blt(save, bx, by, save, -(bw / 2) + 15 * bw,
                        -(bh / 3) + 15 * bh, -(bw / 2) + 15 * bw + bw - 1,
                        -(bh / 3) + 15 * bh + bh - 1,
    ...
    grx_context_bit_blt(tile, 0, 0, save, -(bw / 2) + 15 * bw, -(bh / 3) + 15 * bh,
        -(bw / 2) + 15 * bw + bw - 1, -(bh / 3) + 15 * bh + bh - 1,
        GRX_COLOR_MODE_WRITE);
    grx_set_current_context(tile);
    grx_draw_framed_box(
        2 * wdt, 2 * wdt, bw - 2 * wdt - 1, bh - 2 * wdt - 1, 2 * wdt, &bcolors);
    ...
    for (ii = 0, by = -(bh / 3); ii < 19; ii++) {
        for (jj = 0, bx = -(bw / 2); jj < 19; jj++) {
            grx_context_bit_blt(
                save, bx, by, tile, 0, 0, bw - 1, bh - 1, GRX_COLOR_MODE_WRITE);
            bx += bw;
        }
        by += bh;
    }
    ...
    run_main_loop_until_key_press();
    grx_reset_clip_box();
    grx_bit_blt(60, 60, NULL, 20, 20, grx_get_width() - 40, grx_get_height() - 40,
        GRX_COLOR_MODE_WRITE);

    run_main_loop_until_key_press();

    grx_bit_blt(10, 10, NULL, 60, 60, grx_get_width() - 40, grx_get_height() - 40,
        GRX_COLOR_MODE_WRITE);
    ...
    grx_context_bit_blt(tile, 0, 0, save, -(bw / 2), -(bh / 3),
        -(bw / 2) + 15 * bw + bw - 1, -(bh / 3) + 15 * bh + bh - 1,
        GRX_COLOR_MODE_WRITE);
