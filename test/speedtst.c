...
void drawpixeltest(gvmode *gp, XY_PAIRS *pairs)
{
    ...
    for (i = 0; i < DRAWPIX_loops; ++i) {
        for (j = PAIRS - 1; j >= 0; j--)
            grx_fast_draw_pixel(x[j], y[j], c1);
        for (j = PAIRS - 1; j >= 0; j--)
            grx_fast_draw_pixel(x[j], y[j], c2);
        for (j = PAIRS - 1; j >= 0; j--)
            grx_fast_draw_pixel(x[j], y[j], c3);
        for (j = PAIRS - 1; j >= 0; j--)
            grx_fast_draw_pixel(x[j], y[j], c4);
    }
    t2 = g_get_monotonic_time();
    seconds = (double)(t2 - t1) / 1000000.0;
    if (seconds > 0)
...
void drawblocktest(gvmode *gp, XY_PAIRS *pairs)
{
    ...
    if (!MEASURED(gp)) {
        ...
        for (j = 0; j < PAIRS; j += 2)
            gp->drawblk.count +=
                abs_diff(xb[j], xb[j + 1]) * abs_diff(yb[j], yb[j + 1]);
...
void xor_draw_blocks(GrxContext *c)
{
    ...
    for (i = 28; i > 1; --i)
        grx_draw_filled_box(grx_get_max_x() / i, grx_get_max_y() / i,
            (i - 1) * grx_get_max_x() / i, (i - 1) * grx_get_max_y() / i,
            GRX_COLOR_WHITE | GRX_COLOR_MODE_XOR);
    grx_set_current_context(&save);
}
    ...
    gvmode *gp, perfm *p, int *xb, int *yb, GrxContext *dst, GrxContext *src)
{
    ...
    for (i = 0; i < BLIT_loops; ++i) {
        for (j = PAIRS - 3; j >= 0; j -= 3)
            grx_context_bit_blt(dst, xb[j + 2], yb[j + 2], src, xb[j + 1], yb[j + 1],
                xb[j], yb[j], GRX_COLOR_MODE_WRITE);
        for (j = PAIRS - 3; j >= 0; j -= 3)
            grx_context_bit_blt(dst, xb[j + 2], yb[j + 2], src, xb[j + 1], yb[j + 1],
                xb[j], yb[j], GRX_COLOR_MODE_XOR);
        for (j = PAIRS - 3; j >= 0; j -= 3)
            grx_context_bit_blt(dst, xb[j + 2], yb[j + 2], src, xb[j + 1], yb[j + 1],
                xb[j], yb[j], GRX_COLOR_MODE_OR);
        for (j = PAIRS - 3; j >= 0; j -= 3)
            grx_context_bit_blt(dst, xb[j + 2], yb[j + 2], src, xb[j + 1], yb[j + 1],
                xb[j], yb[j], GRX_COLOR_MODE_AND);
    }
    t2 = g_get_monotonic_time();
    seconds = (double)(t2 - t1) / 1000000.0;
    if (seconds > 0)
...
void blittest(gvmode *gp, XY_PAIRS *pairs, int ram)
{
    ...
    if (!MEASURED(gp)) {
        ...
        gp->blitv2v.count = gp->blitr2v.count = gp->blitv2r.count =
            count * 4.0 * BLIT_loops;
        gp->blitv2v.rate = gp->blitr2v.rate = gp->blitv2r.rate = 0.0;
    }
...
int identical_measured(gvmode *tm)
{
    int i;
    for (i = 0; i < n_modes; ++i) {
        if (tm != &rammodes[i] && tm->fm == rammodes[i].fm && tm->w == rammodes[i].w
            && tm->h == rammodes[i].h && tm->bpp == rammodes[i].bpp
            && MEASURED(&rammodes[i]))
            return 1;
    }
    return 0;
...
void speedcheck(gvmode *gp, int print, int wait)
{
    ...
    grx_set_mode(
        GRX_GRAPHICS_MODE_GRAPHICS_WIDTH_HEIGHT_BPP, NULL, gp->w, gp->h, gp->bpp);
...
void ModeText(int i, int shrt, char *mdtxt)
{
    ...
    switch (shrt) {
    case 2:
        sprintf(mdtxt, "%2d%s %dx%d ", i + 1, flg, grmodes[i].w, grmodes[i].h);
        break;
    case 1:
        sprintf(mdtxt, "%2d%s %4dx%-4d ", i + 1, flg, grmodes[i].w, grmodes[i].h);
        break;
    default:
        sprintf(mdtxt, "  %2d%s  %4dx%-4d ", i + 1, flg, grmodes[i].w, grmodes[i].h);
        break;
    ...
    do {
        for (shrt = 0; shrt <= 2; ++shrt) {
            ...
            for (i = 0; i < n_modes; ++i) {
                ...
                if (strlen(mdtxt) > maxlen)
                    maxlen = strlen(mdtxt);
            }
            n = 2;
            if (cols > 1 || shrt < 2) {
                if (!ColsCheck(cols, maxlen, n))
                    continue;
                while (ColsCheck(cols, maxlen, n + 1) && n < 4)
                    ++n;
            }
            c = 0;
            for (i = 0; i < n_modes; ++i) {
                if (++c == cols)
                    c = 0;
                ModeText(i, shrt, mdtxt);
                printf("%*s%s", (c ? -((int)(maxlen + n)) : -((int)maxlen)), mdtxt,
                    (c || (i + 1 == n_modes) ? "" : "\n"));
            }
            return;
        }
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        printf(
            ...
            grx_get_current_video_driver()->name, GrDriverInfo->defgw,
            GrDriverInfo->defgh, (long)GrDriverInfo->defgc);
        PrintModes();
