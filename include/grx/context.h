...
GrxContext *grx_context_new_full(
    GrxFrameMode mode, gint w, gint h, guint8 *memory, GrxContext *where);
GrxContext *grx_context_new_subcontext(
    gint x1, gint y1, gint x2, gint y2, const GrxContext *parent, GrxContext *where);
void grx_context_resize_subcontext(
    GrxContext *context, gint x1, gint y1, gint x2, gint y2);
...
void grx_context_flood_spill(GrxContext *context, gint x1, gint y1, gint x2, gint y2,
    GrxColor old_c, GrxColor new_c);
void grx_context_flood_spill2(GrxContext *context, gint x1, gint y1, gint x2, gint y2,
    GrxColor old_c1, GrxColor new_c1, GrxColor old_c2, GrxColor new_c2);
GrxColor grx_context_get_pixel_at(GrxContext *context, gint x, gint y);
void grx_context_bit_blt(GrxContext *context, gint x, gint y, GrxContext *src, gint x1,
    gint y1, gint x2, gint y2, GrxColor op);
void grx_context_bit_blt_1bpp(GrxContext *context, gint x, gint y, GrxContext *src,
    gint x1, gint y1, gint x2, gint y2, GrxColor fg, GrxColor bg);
const GrxColor *grx_context_get_scanline(
    GrxContext *context, gint x1, gint x2, gint y, guint *n);

void grx_context_set_clip_box(GrxContext *context, gint x1, gint y1, gint x2, gint y2);
void grx_context_get_clip_box(
    const GrxContext *context, gint *x1, gint *y1, gint *x2, gint *y2);
...
#define grx_context_get_clip_box(C, x1p, y1p, x2p, y2p) \
    do {                                                \
        *(x1p) = (C)->x_clip_low;                       \
        *(y1p) = (C)->y_clip_low;                       \
        *(x2p) = (C)->x_clip_high;                      \
