...
void GrPatternedPolyLine(int numpts,int points[][2],GrLinePattern *lp)
{
        ...
        _GrDrawCustomPolygon(4,points,lp->lnp_option,grf,fval,FALSE,FALSE);
        free(grf);
}
...
void GrPatternedPolygon(int numpts,int points[][2],GrLinePattern *lp)
{
        ...
        grf->line = _GrDrawPatternedLine;
        grf->scan = _GrFillPatternedScanLine;
        _GrDrawCustomPolygon(numpts,points,lp->lnp_option,grf,fval,TRUE,FALSE);
        free(grf);
}
...
void GrPatternedBox(int x1,int y1,int x2,int y2,GrLinePattern *lp)
{
        ...
        grf->line = _GrDrawPatternedLine;
        grf->scan = _GrFillPatternedScanLine;
        _GrDrawCustomPolygon(4,points,lp->lnp_option,grf,fval,TRUE,FALSE);
        free(grf);
}
