...
static void shrink(
    unsigned char *ptr, int pitch, unsigned int oldlen, unsigned int newlen)
{
    ...
    do {
        ...
        do {
            unsigned int factor;
            if (weight == 0)
                weight = newlen, src += pitch;
            if ((factor = weight) > collect)
                factor = collect;
            pixsum += factor * (*src);
            weight -= factor;
            collect -= factor;
...
void grx_resize_gray_map(unsigned char *map, int pitch, int ow, int oh, int nw, int nh)
{
    if (ow != nw) {
        ...
        int cnt = oh;
        if ((unsigned int)ow > (unsigned int)nw)
            do {
                shrink(ptr, 1, ow, nw);
                ptr += pitch;
            } while (--cnt > 0);
    ...
    if (oh != nh) {
        int cnt = nw;
        if ((unsigned int)oh > (unsigned int)nh)
            do {
