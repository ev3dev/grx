...
static int setmode(GrxVideoMode *mp, int noclear)
{
    ...
    if (_XGrPatternGC != None)
        XFreeGC(_XGrDisplay, _XGrPatternGC);
    if (_XGrPattern != None)
        XFreePixmap(_XGrDisplay, _XGrPattern);
    if (_XGrBitmapGC != None)
        XFreeGC(_XGrDisplay, _XGrBitmapGC);
    if (_XGrBitmap != None)
        XFreePixmap(_XGrDisplay, _XGrBitmap);
    if (_XGrGC != None)
    ...
    if (mp->mode != 0) {
        ...
        XGrabKeyboard(
            _XGrDisplay, _XGrWindow, True, GrabModeAsync, GrabModeAsync, CurrentTime);
        XGrabPointer(_XGrDisplay, _XGrWindow, True,
            StructureNotifyMask | ButtonPressMask | ButtonReleaseMask | ButtonMotionMask
                | PointerMotionMask,
        ...
        if (mp->bpp == 8) {
            if (_XGrColormap == None) {
                ...
                _XGrColormap =
                    XDGACreateColormap(_XGrDisplay, _XGrScreen, dev, AllocAll);
            }
            if (_XGrColormap != None) {
...
#ifdef XF86DGA_FRAMEBUFFER
        if (!noclear)
            memzero(dev->data, mp->line_offset * mp->height);
#else
        ...
        if (!noclear) {
            ...
            XFillRectangle(
                _XGrDisplay, dev->pixmap, _XGrGC, 0, 0, mp->width, mp->height);
        }
#endif
    }
    res = TRUE;
...
static int build_video_mode(XDGAMode *ip, GrxVideoMode *mp, GrxVideoModeExt *ep)
{
    ...
    case 15:
        ...
        ep->cprec[0] = ep->cprec[1] = ep->cprec[2] = 5;
        ep->cpos[0] = 10;
        ep->cpos[1] = 5;
        ep->cpos[2] = 0;
        break;
    case 16:
        ...
        break;
    case 24:
        ep->cprec[0] = ep->cprec[1] = ep->cprec[2] = 8;
        ep->cpos[0] = 16;
        ep->cpos[1] = 8;
        ep->cpos[2] = 0;
...
static GrxVideoMode modes[NUM_MODES] = {
    ...
    { TRUE, 8, 80, 25, 0x00, 80, 1, &grtextextdga }, { 0 }
};
...
static int init(char *options)
{
    ...
    if (detect()) {
        ...
        for (mindex = 0; mindex < modec; mindex++) {
            if (!build_video_mode(&modev[mindex], &mode, &ext))
                continue;
