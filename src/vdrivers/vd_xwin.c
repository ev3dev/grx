...
char *_XGrClassNames[6] = { "StaticGray", "GrayScale", "StaticColor", "PseudoColor",
...
static GrxVideoModeExt grxwinext = {
    .mode = GRX_FRAME_MODE_UNDEFINED, /* frame driver */
    .drv = NULL,                      /* frame driver override */
    .frame = NULL,                    /* frame buffer address */
    .cprec = { 0, 0, 0 },             /* color precisions */
    .cpos = { 0, 0, 0 },              /* color component bit positions */
    ...
    .set_virtual_size = NULL,         /* virtual size set */
    .scroll = NULL,                   /* virtual scroll */
    .set_bank = set_bank,             /* bank set function */
    .set_rw_banks = set_rw_banks,     /* double bank set */
...
static void load_color(int c, int r, int g, int b)
{
    ...
    if (_XGrDisplay != NULL && _XGrColormap != None
        ...
        DBGPRINTF(
            DBG_DRIVER, ("Setting X11 color %d to r=%d, g=%d, b=%d\n", c, r, g, b));
        xcolor.pixel = c;
        xcolor.red = r * 257;
        xcolor.green = g * 257;
        xcolor.blue = b * 257;
    ...
    if (_XGrWindow != None) {
        ...
        if (mp->extended_info->mode != GRX_FRAME_MODE_TEXT) {
            ...
            if (USE_PIXMAP_FOR_BS) {
                _XGrBStore = XCreatePixmap(
                    _XGrDisplay, _XGrWindow, mp->width, mp->height, _XGrDepth);
                _XGrGC = XCreateGC(_XGrDisplay, _XGrBStore, 0L, NULL);
                _XGrBStoreInited = 1;
                XFillRectangle(
                    _XGrDisplay, _XGrBStore, _XGrGC, 0, 0, mp->width, mp->height);
...
static int error_handler(Display *dpy, XErrorEvent *ev)
{
    ...
    XGetErrorText(dpy, ev->error_code, buffer, sizeof(buffer)),
        fprintf(stderr, "GRX: XError: %s XID=%ld request_code=%d serial=%lu\n", buffer,
            ev->resourceid, ev->request_code, ev->serial);
    return 0;
}

static GrxVideoMode *_xw_selectmode(
...
static int init(const char *options)
{
    ...
    attr.event_mask =
        (StructureNotifyMask | KeyPressMask | KeyReleaseMask | ButtonPressMask
            | ButtonReleaseMask | ButtonMotionMask | ExposureMask | PointerMotionMask);
    mask |= CWEventMask;
    _XGrWindow = XCreateWindow(_XGrDisplay, root, /* parent */
        0,                                        /* x */
        0,                                        /* y */
        1,                                        /* width */
        1,                                        /* height */
    ...
    switch (depth) {
    case 1:
        grxwinext.mode = GR_frameXWIN1;
        break;
    case 4:
        grxwinext.mode = GR_frameXWIN4;
        break;
    case 8:
        grxwinext.mode = GR_frameXWIN8;
        break;
    case 15:
    case 16:
        grxwinext.mode = GR_frameXWIN16;
        break;
    case 24:
        switch (bpp) {
        case 24:
            grxwinext.mode = GR_frameXWIN24;
            break;
        case 32:
            grxwinext.mode =
                (visual->red_mask & 0xff000000) ? GR_frameXWIN32H : GR_frameXWIN32L;
            break;
        }
        break;
    }
    ...
    for (mp = &modes[1]; mp < &modes[itemsof(modes) - 1]; mp++) {
        ...
        if (mp->width > _XGrMaxWidth)
            mp->present = FALSE;
        if (mp->height > _XGrMaxHeight)
            mp->present = FALSE;
    }
...
GrxVideoDriver _GrVideoDriverXWIN = {
    ...
    .select_mode = _xw_selectmode,                 /* special mode select routine */
