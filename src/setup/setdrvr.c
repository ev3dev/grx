...
static const gchar *const driver_names[] = { "gtk3", "linuxfb", "memory", NULL };
...
#define CH16(c1, c2) (((c1) << 8) | (c2))
        ...
        if (name[0] != '\0') {
            ...
            if (!g_module_symbol(plugin, module_name, (gpointer *)&drv)) {
                ...
                g_set_error(error, GRX_ERROR, GRX_ERROR_PLUGIN_SYMBOL_NOT_FOUND,
                    "Could not symbol '%s' in plugin '%s'", module_name, name);
                return FALSE;
            }
        }
    }
    ...
    g_set_error(
        error, GRX_ERROR, GRX_ERROR_FAILED, "Driver '%s' failed to init", drv->name);
