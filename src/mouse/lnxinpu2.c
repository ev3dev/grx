 ...
 **
 ** Handles the mouse directly, but only PS/2 (M.Alvarez 07/02/2002)
 **
 **/
...
//            mou_filedsc = open("/dev/psaux",O_RDONLY);
            if(mou_filedsc >= 0) {
...
static int _GrReadPS2MouseData(int *mb,int *mx,int *my)
{
        ...
        *mx = *my = 0;
        do {
            if(read(mou_filedsc,buf,1)<1) return evr;
            if(!(buf[0] & 0x08)) return evr; /* sync lost */
