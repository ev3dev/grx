...
void _XGrCopyBStore(int x, int y, int width, int lenght)
{
    if (USE_PIXMAP_FOR_BS) {
        XCopyArea(_XGrDisplay, _XGrBStore, _XGrWindow,
            DefaultGC(_XGrDisplay, _XGrScreen), x, y, width, lenght, x, y);
    }
}

static INLINE void _XGrSetForeColor(GrxColor color)
{
    GRX_ENTER();
...
static INLINE void _XGrSetBackColor(GrxColor color)
{
    GRX_ENTER();
    color &= GRX_COLOR_VALUE_MASK;
...
static INLINE void _XGrSetColorOper(GrxColor color)
{
...
#define AREA_OVERLAP_PIXEL_CACHE(y1, y2) \
    ((y2) >= _XGrPixelCacheY1 && (y1) <= _XGrPixelCacheY2)

#define PIXEL_CACHE_INVALIDATE()                \
    do {                                        \
...
static INLINE void _XGrCheckPixelCache(int y1, int y2)
{
...
static GrxColor readpixel(GrxFrame *c, int x, int y)
{
    GrxColor col;
    GRX_ENTER();
    if (_XGrPixelCacheDrawable != (Drawable)c->base_address
        || _XGrPixelCacheImage == NULL || !AREA_OVERLAP_PIXEL_CACHE(y, y)) {
        if (_XGrPixelCacheImage) {
            XDestroyImage(_XGrPixelCacheImage);
            _XGrPixelCacheImage = NULL;
        }
        _XGrPixelCacheDrawable = (Drawable)c->base_address;

        _XGrPixelCacheWidth = grx_get_screen_width();
        _XGrPixelCacheY1 = y;
        _XGrPixelCacheY2 = y + PIXEL_CACHE_HEIGHT - 1;
        if (_XGrPixelCacheY2 >= grx_get_screen_height())
            _XGrPixelCacheY2 = grx_get_screen_height() - 1;
        _XGrPixelCacheHeight = _XGrPixelCacheY2 - _XGrPixelCacheY1 + 1;

        _XGrPixelCacheImage =
            XGetImage(_XGrDisplay, _XGrPixelCacheDrawable, 0, _XGrPixelCacheY1,
                _XGrPixelCacheWidth, _XGrPixelCacheHeight, AllPlanes, ZPixmap);
        if (!_XGrPixelCacheImage) {
            col = GRX_COLOR_NONE;
            goto done;
        }
...
static INLINE void drawpixel(int x, int y, GrxColor c)
{
    GRX_ENTER();
    _XGrSetForeColor(c);
    _XGrSetColorOper(c);
    XDrawPoint(_XGrDisplay, (Drawable)CURC->gc_base_address, _XGrGC, x, y);
...
static void drawhline(int x, int y, int w, GrxColor c)
{
    int x2;
    ...
    _XGrSetColorOper(c);
    x2 = x + w - 1;
    XDrawLine(_XGrDisplay, (Drawable)CURC->gc_base_address, _XGrGC, x, y, x2, y);
...
static void drawvline(int x, int y, int h, GrxColor c)
{
    int y2;
    ...
    _XGrSetColorOper(c);
    y2 = y + h - 1;
    XDrawLine(_XGrDisplay, (Drawable)CURC->gc_base_address, _XGrGC, x, y, x, y2);
...
static void drawblock(int x, int y, int w, int h, GrxColor c)
{
    ...
    _XGrSetForeColor(c);
    _XGrSetColorOper(c);
    XFillRectangle(_XGrDisplay, (Drawable)CURC->gc_base_address, _XGrGC, x, y, w, h);
...
static void drawline(int x, int y, int dx, int dy, GrxColor c)
{
    ...
    dx += x;
    dy += y;
    XDrawLine(_XGrDisplay, (Drawable)CURC->gc_base_address, _XGrGC, x, y, dx, dy);
    isort(x, dx);
    isort(y, dy);
    ...
    int start, GrxColor fg, GrxColor bg)
{
...
#endif
    {
        if ((C_OPER(fg) == C_OPER(bg)) && (fg != GRX_COLOR_NONE)) {
            ...
            XPutImage(_XGrDisplay, (Drawable)CURC->gc_base_address, _XGrGC, &ximage, 0,
                0, x, y, w, h);
            ...
            if (fg != GRX_COLOR_NONE) {
                ...
                XPutImage(
                    _XGrDisplay, _XGrBitmap, _XGrBitmapGC, &ximage, 0, 0, 0, 0, w, h);
                ...
                _XGrSetColorOper(fg);
                XFillRectangle(
                    _XGrDisplay, (Drawable)CURC->gc_base_address, _XGrGC, x, y, w, h);
            }
    ...
    int x, int y, int w, unsigned char patt, GrxColor fg, GrxColor bg)
{
...
#endif
    {
        if ((C_OPER(fg) == C_OPER(bg)) && (fg != GRX_COLOR_NONE)) {
            ...
            XPutImage(
                _XGrDisplay, _XGrPattern, _XGrPatternGC, &ximage, 0, 0, 0, 0, 8, 1);
            ...
            _XGrSetColorOper(fg);
            XFillRectangle(
                _XGrDisplay, (Drawable)CURC->gc_base_address, _XGrGC, x, y, w, 1);
        }
        else {
            if (fg != GRX_COLOR_NONE) {
                XSetForeground(_XGrDisplay, _XGrPatternGC, 1);
    ...
    int h, GrxColor op)
{
    GRX_ENTER();
    _XGrSetColorOper(op);
    XCopyArea(_XGrDisplay, (Drawable)src->base_address, (Drawable)dst->base_address,
    ...
    int h, GrxColor op)
{
    ...
    if (grx_color_get_mode(op) == GRX_COLOR_MODE_IMAGE)
        ...
        ximage = XGetImage(
            _XGrDisplay, (Drawable)src->base_address, sx, sy, w, h, AllPlanes, ZPixmap);
        if (ximage) {
            int bytes_per_pixel = _XGrBitsPerPixel >> 3;
            GrxFrame tmp = *dst;

            tmp.base_address = ximage->data;
    ...
    int h, GrxColor op)
{
    ...
    if (grx_color_get_mode(op) == GRX_COLOR_MODE_IMAGE)
        ...
        {
            _XGrSetColorOper(op);
            XPutImage(_XGrDisplay, (Drawable)dst->base_address, _XGrGC, &ximage, sx, sy,
                dx, dy, w, h);
            _XGrCopyBStore(dx, dy, w, h);
            _XGrCheckPixelCache(dy, dy + h - 1);
        }
...
static void putscanline(int x, int y, int w, const GrxColor *scl, GrxColor op)
{
    ...
    for (ind = 0; ind < w; ind++) {
        if (scl[ind] != skipc) {
            ...
            XDrawPoint(
                _XGrDisplay, (Drawable)CURC->gc_base_address, _XGrGC, x + ind, y);
        }
    }
...
#define ROUNDCOLORCOMP(x, n)                            \
    (((uint)(x) >= CLRINFO->mask[n]) ? CLRINFO->mask[n] \
