...
static void drawvline(int x, int y, int h, GrxColor color)
{
    ...
    case C_XOR:
        ...
        break;
    case C_OR:
        colfill32_or(pp, lwdt, color, h);
        break;
    case C_AND:
        colfill32_and(pp, lwdt, color, h);
        break;
    default:
        colfill32(pp, lwdt, color, h);
        break;
    }
    GRX_LEAVE();
}
...
static void drawhline(int x, int y, int w, GrxColor color)
{
    ...
    case C_XOR:
        ...
        break;
    case C_OR:
        repfill32_or(pp, cval, w);
        break;
    case C_AND:
        repfill32_and(pp, cval, w);
        break;
    default:
        repfill32(pp, cval, w);
        break;
...
static void drawblock(int x, int y, int w, int h, GrxColor color)
{
    ...
    pp = &CURC->gc_base_address[FOFS(x, y, skip)];
    skip -= w << 2;
    op = C_OPER(color);
    color = COL2PIX(color);
    cval = freplicate_l(color);
...
#define ASM_LINE1(OPC)                                    \
    asm volatile(                                         \
        ...
        : "0"((long)(ptr)), "1"(npts), "2"(error), "r"((long)(color)), "o"(lndata))
