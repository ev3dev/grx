...
void _GrFrDrvPackedBitBltV2R(GrxFrame *dst, int dx, int dy, GrxFrame *src, int sx,
    int sy, int w, int h, GrxColor op)
{
    ...
    GRX_ENTER();
    dptr = &dst->base_address[FOFS(dx, dy, dst->line_offset)];
...
#define DOICPY() DOIMGCOPY(FW, _n, _f, w1)
    ...
    do {
        ...
        do {
            sptr = &src->base_address[BANKPOS(soff)];
            ...
            if (w1)
                ...
                case C_IMAGE:
                    ...
                    break;
                case C_XOR:
                    fwdcopy_xor_f(sptr, dptr, sptr, w1);
                    break;
                case C_OR:
                    fwdcopy_or_f(sptr, dptr, sptr, w1);
                    break;
                case C_AND:
                    fwdcopy_and_f(sptr, dptr, sptr, w1);
                    break;
                default:
                    fwdcopy_set_f(sptr, dptr, sptr, w1);
                    break;
                }
            w1 = w2;
            w2 = 0;
