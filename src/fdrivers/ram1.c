...
static INLINE void drawpixel(int x, int y, GrxColor color)
{
    ...
    case C_AND:
        *ptr &= ~cval;
        break;
    default:
        *ptr = (*ptr & (~0x80 >> (7 - x))) | cval;
        break;
    }
    GRX_LEAVE();
}

static INLINE void drawpixel_inv(int x, int y, GrxColor color)
{
    drawpixel(x, y, grx_color_get_mode(color) | grx_color_get_value(~color));
}

#define maskoper(d, op, s, msk, SF, DF)                          \
    do {                                                         \
        unsigned char _c_ = peek_b##DF(d);                       \
        poke_b##DF((d), (_c_ & ~(msk)) | ((_c_ op(s)) & (msk))); \
    } while (0)
#define maskset(d, c, msk, DF) poke_b##DF((d), (peek_b##DF(d) & ~(msk)) | ((c) & (msk)))
...
static void drawhline(int x, int y, int w, GrxColor color)
{
    ...
    if (!(!color && (oper == C_OR || oper == C_XOR)) && !(color && oper == C_AND)) {
        ...
        GR_int8u *p =
            (GR_int8u *)&CURC->gc_base_address[FOFS(x, y, CURC->gc_line_offset)];
        GR_repl cv = 0;
        if (color)
            cv = ~cv;
        w = ((x + w + 7) >> 3) - (x >> 3);
        ...
        if (((GR_int8u)(~lm))) {
            switch (oper) {
            case C_XOR:
                maskoper(p, ^, (GR_int8u)cv, lm, _set, _n);
                break;
            case C_OR:
                maskoper(p, |, (GR_int8u)cv, lm, _set, _n);
                break;
        ...
        if (w) {
            switch (oper) {
            case C_XOR:
                repfill_b_xor(p, cv, w);
                break;
            case C_OR:
                repfill_b_or(p, cv, w);
                break;
            case C_AND:
                repfill_b_and(p, cv, w);
                break;
            default:
                repfill_b(p, cv, w);
                break;
        ...
        if (((GR_int8u)(~rm))) {
            switch (oper) {
            case C_XOR:
                maskoper(p, ^, (GR_int8u)cv, rm, _set, _n);
                break;
            case C_OR:
                maskoper(p, |, (GR_int8u)cv, rm, _set, _n);
                break;
