...
/**
 * grx_user_draw_filled_polygon_with_pattern:
 ...
 */
void grx_user_draw_filled_polygon_with_pattern(int numpts,GrxPoint *points,GrxPattern *p)
{
        int pt;
        GrxPoint *tmp;
        setup_ALLOC();
        ...
        if (tmp != NULL) {
          ...
          grx_draw_filled_polygon_with_pattern(numpts,tmp,p);
          FREE(tmp);
        }
        reset_ALLOC();
}
