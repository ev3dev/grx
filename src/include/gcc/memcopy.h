...
#define __INLINE_386_G_COPY__(DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL)  \
    ({                                                                              \
        ...
        __asm__ volatile(                                                           \
            ...
            "=&q"((TYPE)_scr_)                                                      \
            : "0"((void *)(DP)), "1"((void *)(SP)), "2"((int)(C)), "gn"((int)(DS)), \
            "gn"((int)(SS))                                                         \
            : "memory");                                                            \
    })

#define __INLINE_386_C_COPY__(                                        \
    DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL, DINC, SINC)  \
    ({                                                                \
        register TYPE _scr_;                                          \
        __asm__ volatile(                                             \
            ""                                                        \
...
#define __INLINE_386_C2_COPY__(                                                        \
    DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL, DINC)                         \
    ({                                                                                 \
        if ((SS) == 1)                                                                 \
            __INLINE_386_C_COPY__(                                                     \
                DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL, DINC, "incl %1"); \
        else if ((SS) == -1)                                                           \
            __INLINE_386_C_COPY__(                                                     \
                DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL, DINC, "decl %1"); \
        else if ((SS) == 2)                                                            \
            __INLINE_386_C_COPY__(DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL, \
                DINC, "leal 2(%1),%1");                                                \
        else if ((SS) == -2)                                                           \
            __INLINE_386_C_COPY__(DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL, \
                DINC, "leal -2(%1),%1");                                               \
            ...
            __INLINE_386_G_COPY__(                                                     \
                DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL);                  \
    })

#define __INLINE_386_C1_COPY__(DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL)    \
    ({                                                                                 \
        if ((DS) == 1)                                                                 \
            __INLINE_386_C2_COPY__(                                                    \
                DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL, "incl %0");       \
        else if ((DS) == -1)                                                           \
            __INLINE_386_C2_COPY__(                                                    \
                DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL, "decl %0");       \
        else if ((DS) == 2)                                                            \
            __INLINE_386_C2_COPY__(                                                    \
                DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL, "leal 2(%0),%0"); \
        else if ((DS) == -2)                                                           \
            __INLINE_386_C2_COPY__(DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL,      \
                SSEL, "leal -2(%0),%0");                                               \
        else if ((DS) == 4)                                                            \
            __INLINE_386_C2_COPY__(                                                    \
                DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL, SSEL, "leal 4(%0),%0"); \
        else if ((DS) == -4)                                                           \
            __INLINE_386_C2_COPY__(DP, DS, SP, SS, C, DIN, SIN, SIZE, TYPE, DSEL,      \
                SSEL, "leal -4(%0),%0");                                               \
        else                                                                           \
