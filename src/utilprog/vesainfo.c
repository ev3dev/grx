...
bitdef modeattrbits[] = { DEFBIT(MODE_SUPPORTED), DEFBIT(MODE_EXTINFO),
    DEFBIT(MODE_SUPBIOS), DEFBIT(MODE_ISCOLOR), DEFBIT(MODE_ISGRAPHICS),
    /* ==== VESA 2.0 and later ==== */
    DEFBIT(MODE_NON_VGA), DEFBIT(MODE_NO_BANKING), DEFBIT(MODE_LIN_FRAME), { 0 } };

bitdef winattrbits[] = { DEFBIT(WIN_SUPPORTED), DEFBIT(WIN_READABLE),
    DEFBIT(WIN_WRITABLE), { 0 } };

bitdef memorymodels[] = { DEFBIT(MODEL_TEXT), DEFBIT(MODEL_CGA), DEFBIT(MODEL_HERC),
    DEFBIT(MODEL_4PLANE), DEFBIT(MODEL_PACKED), DEFBIT(MODEL_256_NC),
    DEFBIT(MODEL_DIRECT), { 0 } };

bitdef capabilitiesbits[] = { DEFBIT(CAP_DAC_WIDTH),
    /* ==== VESA 2.0 and later ==== */
    DEFBIT(CAP_NON_VGA), DEFBIT(CAP_DAC_BLANK), { 0 } };
...
void printinfo(VESAvgaInfoBlock *vgainfo)
{
    ...
    printf("VESASignature:\t\"%c%c%c%c\"\n", vgainfo->VESAsignature[0],
        vgainfo->VESAsignature[1], vgainfo->VESAsignature[2],
...
char *getmodelname(int model)
{
    ...
    if (model < 0) {
        sprintf(temp, "Invalid model [%d]", model);
        return temp;
    }
    if (model <= MODEL_DIRECT) {
        return memorymodels[model].name;
    }
    if (model <= 15) {
        sprintf(temp, "VESA model [0x%02x]", model);
        return temp;
    }
    sprintf(temp, "OEM model [%0x2x]", model);
    return temp;
}
...
void printmodeinfo(int mode, int version, VESAmodeInfoBlock *modeinfo)
{
    ...
    if (!(modeinfo->ModeAttributes & MODE_EXTINFO))
        return;
    ...
    printf("  MemoryModel:      %d (%s)\n", modeinfo->MemoryModel,
        getmodelname(modeinfo->MemoryModel));
    printf("  BankSize:         %d\n", modeinfo->BankSize);
    printf("  NumImagePages     %d\n", modeinfo->NumImagePages);
    if (version < VESA_VERSION(1, 2))
        return;
...
{
    unsigned short *st =
        (unsigned short *)PTR_ADD(pb, pb->SubTable_off + VESApmInfoBlock_BASEOFF);
    ...
    printf("  set window offset:\t\t%04x (%04x:%04x)\n", pb->SetWindow_off,
        pb->RealMode_SEG, pb->RealMode_OFF + pb->SetWindow_off);
    printf("  set display start offset:\t%04x (%04x:%04x)\n", pb->DisplStart_off,
        pb->RealMode_SEG, pb->RealMode_OFF + pb->DisplStart_off);
    printf("  set primary palette offset:\t%04x (%04x:%04x)\n", pb->PPalette_off,
        pb->RealMode_SEG, pb->RealMode_OFF + pb->PPalette_off);
    printf("  resource table offset:\t%04x (%04x:%04x)\n", pb->SubTable_off,
        pb->RealMode_SEG, pb->RealMode_OFF + pb->SubTable_off);
...
int main(void)
{
    ...
    if (_GrViDrvVESAgetVGAinfo(&vb)) {
        ...
        printf("VESA BIOS extensions not found\n");
    return 0;
}
