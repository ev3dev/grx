...
static size_t inputread(void *buffer, size_t size, size_t number, inputstruct *is)
{
    if (is->method == 0)
...
static gboolean loaddata(
    inputstruct *is, GrxPnmFormat *format, int *width, int *height, int *maxval)
{
...
static gboolean _GrLoadContextFromPbm(inputstruct *is, int width, int height)
{
    ...
    for (y = 0; y < maxheight; y++) {
        for (x = 0; x < width; x++) {
            if (!isbyteread) {
                if (inputread(&currentbyte, 1, 1, is) != 1) {
                    res = FALSE;
                    goto out;
                }
                isbyteread = 1;
            ...
            if (x < maxwidth) {
                color =
                    currentbyte & (1 << currentbit) ? GRX_COLOR_BLACK : GRX_COLOR_WHITE;
                pColors[x] = color;
            }
            currentbit--;
            if (currentbit < 0)
...
static gboolean _GrLoadContextFromPgm(
    inputstruct *is, int width, int height, int maxval)
{
    int x, y;
    ...
    for (y = 0; y < maxheight; y++) {
        if (inputread(pData, 1, width, is) != width) {
            res = FALSE;
            goto out;
        }
        pCursor = pData;
...
static gboolean _GrLoadContextFromPpm(
    inputstruct *is, int width, int height, int maxval)
{
    int x, y;
    int needcoloradjust = 0;
    ...
    for (y = 0; y < maxheight; y++) {
        if (inputread(pRGB, 3, width, is) != width) {
            res = FALSE;
            goto out;
        }
        pCursor = pRGB;
...
gboolean grx_context_load_from_pnm(GrxContext *grc, const char *pnmfn, GError **error)
{
    ...
    switch (format) {
    case GRX_PNM_FORMAT_BINARY_PBM:
        r = _GrLoadContextFromPbm(&is, width, height);
        break;
    case GRX_PNM_FORMAT_BINARY_PGM:
        r = _GrLoadContextFromPgm(&is, width, height, maxval);
        break;
    case GRX_PNM_FORMAT_BINARY_PPM:
        r = _GrLoadContextFromPpm(&is, width, height, maxval);
        break;
    default:
        break;
    }
 ...
 * Returns: %TRUE on success, otherwise %FALSE
 */
gboolean grx_query_pnm_file(
    const char *pnmfn, GrxPnmFormat *format, int *width, int *height, int *maxval)
{
    inputstruct is = { 0, NULL, NULL, 0 };
    GrxPnmFormat r;
 ...
 * Returns: %TRUE on success, otherwise %FALSE
 */
gboolean grx_query_pnm_data(
    GByteArray *pnmbuf, GrxPnmFormat *format, int *width, int *height, int *maxval)
{
    inputstruct is = { 1, NULL, NULL, 0 };
    gboolean r;
