    ...
    int xc, int yc, int rx, int ry, GrFiller *f, GrFillArg c, int filled)
{
    ...
    if ((rx == 0) || (ry == 0))
        (*f->line)(
            (x1 + CURC->x_offset), (y1 + CURC->y_offset), (x2 - x1), (y2 - y1), c);
        ...
        error = (yasq2 * (col + 1) * col) + (xasq2 * ((row * (row - 2)) + 1))
                + (yasq * (1 - xasq2));
        ...
        for (row = y1; row <= y2; row++) {
            ...
            if (!filled && (col < ry)) {
                ...
                do {
                    ...
                    (*f->scan)((x1 + CURC->x_offset), (row + CURC->y_offset),
                        (x2 - x1 + 1), c);
                } while (0);
            ...
            (*f->scan)((x1 + CURC->x_offset), (row + CURC->y_offset), (x2 - x1 + 1), c);
        }
    }
    mouse_unblock();
}
