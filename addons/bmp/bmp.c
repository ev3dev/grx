...
static unsigned char *__GrLoadPaletteBmp(
    int _handle, unsigned long _paloffset, int _colors);
static unsigned char *GrLoadPaletteBmp(
    int _handle, int *_col, GrBitmapInfoHeader *_iheader);
static char *GrLoadImageFromBmpBiRgb(int _handle, unsigned long _offset,
    long _maxbufsize, int _colors, GrBitmapInfoHeader *_infoheader);
static char *GrLoadImageFromBmpBiRle8(int _handle, unsigned long _offset,
    unsigned long _maxbufsize, int _colors, GrBitmapInfoHeader *_infoheader);
static char *GrLoadImageFromBmpBiRle4(int _handle, unsigned long _offset,
    unsigned long _maxbufsize, int _colors, GrBitmapInfoHeader *_infoheader);
static char *GrLoadImageFromBmp(
    int _handle, unsigned long _offset, int _colors, GrBitmapInfoHeader *_infoheader);
...
GrxPixmap *GrConvertBmpImageToPattern(GrBmpImage *_bmp);
GrxPixmap *GrConvertBmpImageToStaticPattern(GrBmpImage *_bmp);
...
/* ************************************************************************ */
{
    ...
    *_col = -1;
    ...
    case 1: {
        *_col = 2;
        palette = __GrLoadPaletteBmp(_handle, paloffset, 2);
    } break;
    case 4: {
        *_col = 16;
        palette = __GrLoadPaletteBmp(_handle, paloffset, 16);
    } break;
    case 8: {
        *_col = 256;
        palette = __GrLoadPaletteBmp(_handle, paloffset, 256);
    } break;
    case 24: {
        *_col = 0;
        palette = NULL;
...
/* ************************************************************************ */
{
    ...
    case BI_RGB: {
        ...
        map =
            GrLoadImageFromBmpBiRgb(_handle, _offset, maxbufsize, _colors, _infoheader);
        break;
    }
    case BI_RLE8:
        map = GrLoadImageFromBmpBiRle8(
            _handle, _offset, maxbufsize, _colors, _infoheader);
        break;
    case BI_RLE4:
        map = GrLoadImageFromBmpBiRle4(
            _handle, _offset, maxbufsize, _colors, _infoheader);
        break;
    }
    return map;
...
/* ************************************************************************ */
{
    ...
    if (_bmp->bi_palette) {
        ...
        for (i = 0; i < _bmp->bi_numcolors; i++)
            colors[i + 1] = grx_color_get(_bmp->bi_palette[i * 4 + 2],
                _bmp->bi_palette[i * 4 + 1], _bmp->bi_palette[i * 4 + 0]);
...
#define ADD2PTR(p, o) ((void *)((char *)(p) + (o)))
    ...
    if ((handle = open(_filename, BIN_RD)) != -1) {
        bmpimage = malloc(sizeof(GrBmpImage) + sizeof(GrBitmapFileHeader)
                          + sizeof(GrBitmapInfoHeader) + sizeof(GrBmpImageColors));
        if (!bmpimage)
            defClose;
        memset(bmpimage, 0,
            sizeof(GrBmpImage) + sizeof(GrBitmapFileHeader) + sizeof(GrBitmapInfoHeader)
                + sizeof(GrBmpImageColors));
        bmpimage->bi_bmpfileheader = ADD2PTR(bmpimage, sizeof(GrBmpImage));
        bmpimage->bi_bmpinfoheader =
            ADD2PTR(bmpimage, sizeof(GrBmpImage) + sizeof(GrBitmapFileHeader));
        bmpimage->bi_bmpimagecolors =
            ADD2PTR(bmpimage, sizeof(GrBmpImage) + sizeof(GrBitmapFileHeader)
        ...
        if (!GrLoadBmpInfoHeader(handle, bmpimage->bi_bmpinfoheader))
            defClose;
        bmpimage->bi_palette = GrLoadPaletteBmp(
            handle, &(bmpimage->bi_numcolors), bmpimage->bi_bmpinfoheader);
        if (bmpimage->bi_numcolors == -1)
            defClose;
        bmpimage->bi_map = GrLoadImageFromBmp(handle,
            bmpimage->bi_bmpfileheader->bf_offbits - BMPFILEHEADERSIZE,
            bmpimage->bi_numcolors, bmpimage->bi_bmpinfoheader);
        if (!bmpimage->bi_map)
            defClose;
    }
#undef defClose
...
/* ************************************************************************ */
{
    if ((!_bmp) || (!_bmp->bi_map))
        return NULL;
    return grx_pixmap_new(
        _bmp->bi_map, _bmp->bi_width, _bmp->bi_height, _bmp->bi_colormap);
}
...
/* ************************************************************************ */
{
    ...
    if (!_c)
        _c = (GrxContext *)grx_get_current_context();
    ...
    */
    ...
    if (colors == 256)
        fileheader.bf_size =
            BMPINFOHEADERSIZE + BMPFILEHEADERSIZE + 256 * 4 + width * height;
    else
        fileheader.bf_size =
            BMPINFOHEADERSIZE + BMPFILEHEADERSIZE + (width * height * 3);
    ...
    do {
        ...
        do {
            ...
            if (colors == 256)
                ...
                line[(xx * 3) + 0] = grx_color_get_blue(pixcol);
                line[(xx * 3) + 1] = grx_color_get_green(pixcol);
                ;
                line[(xx * 3) + 2] = grx_color_get_red(pixcol);
                ;
            }
        } while (++xx < width);
