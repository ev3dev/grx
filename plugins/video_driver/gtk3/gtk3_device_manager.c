...
G_DEFINE_TYPE_WITH_CODE(GrxGtk3DeviceManager, grx_gtk3_device_manager,
    GRX_TYPE_DEVICE_MANAGER,
    G_IMPLEMENT_INTERFACE(G_TYPE_INITABLE, initable_interface_init))

/* class implementation */

static gboolean free_device_map_item(gpointer key, gpointer value, gpointer user_data)
{
...
static void grx_gtk3_device_manager_class_init(GrxGtk3DeviceManagerClass *klass)
{
    G_OBJECT_CLASS(klass)->finalize = finalize;
}

static void grx_gtk3_device_manager_init(GrxGtk3DeviceManager *self)
{
...
static void on_device_added(
    GdkDeviceManager *device_manager, GdkDevice *device, gpointer user_data)
{
...
static gboolean init(GInitable *initable, GCancellable *cancellable, GError **error)
{
    ...
    if (!display) {
        g_set_error(
            error, G_IO_ERROR, G_IO_ERROR_FAILED, "Failed to get default display");
        return FALSE;
    }
    ...
    self->device_added_signal_id = g_signal_connect(
        self->gdk_device_manager, "device-added", (GCallback)on_device_added, self);
    self->device_removed_signal_id = g_signal_connect(
        self->gdk_device_manager, "device-removed", (GCallback)on_device_removed, self);

    // defer initial "device-added" signals until main loop is started
    devices = gdk_device_manager_list_devices(
        self->gdk_device_manager, GDK_DEVICE_TYPE_MASTER);
