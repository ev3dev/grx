...
G_DEFINE_TYPE_WITH_CODE(GrxLibinputDeviceManager, grx_libinput_device_manager,
    GRX_TYPE_DEVICE_MANAGER,
...
static void grx_libinput_device_manager_class_init(GrxLibinputDeviceManagerClass *klass)
{
...
static void grx_libinput_device_manager_init(GrxLibinputDeviceManager *self) { }
...
static gboolean init(GInitable *initable, GCancellable *cancellable, GError **error)
{
    ...
    if (!self->libinput) {
        g_set_error(
            error, G_IO_ERROR, G_IO_ERROR_FAILED, "Failed to create libinput context.");
        return FALSE;
    }
    ...
    if (!self->xkb) {
        g_set_error(
            error, G_IO_ERROR, G_IO_ERROR_FAILED, "Failed to create xkb context.");
        goto err1;
    }
    ...
    self->keymap =
        xkb_keymap_new_from_names(self->xkb, NULL, XKB_KEYMAP_COMPILE_NO_FLAGS);
    if (!self->keymap) {
        g_set_error(
            error, G_IO_ERROR, G_IO_ERROR_FAILED, "Failed to create xkb keymap.");
        goto err2;
    }
...
#pragma GCC diagnostic ignored "-Wswitch"
    ...
    case LIBINPUT_EVENT_DEVICE_ADDED:
        ...
        g_signal_emit_by_name(instance, "device-added", grx_device);
        break;
    case LIBINPUT_EVENT_DEVICE_REMOVED:
        instance->devices = g_list_remove(instance->devices, grx_device);
        g_signal_emit_by_name(instance, "device-removed", grx_device);
    ...
    case LIBINPUT_EVENT_KEYBOARD_KEY: {
        ...
        case LIBINPUT_KEY_STATE_PRESSED:
            ...
            grx_libinput_device_update_state(grx_device, keycode, XKB_KEY_DOWN,
                &grx_event.key.keysym, &grx_event.key.unichar);
            break;
        case LIBINPUT_KEY_STATE_RELEASED:
            grx_event.type = GRX_EVENT_TYPE_KEY_UP;
            grx_libinput_device_update_state(grx_device, keycode, XKB_KEY_UP,
                &grx_event.key.keysym, &grx_event.key.unichar);
            break;
        }
    ...
    case LIBINPUT_EVENT_POINTER_MOTION_ABSOLUTE: {
        ...
        if (type == LIBINPUT_EVENT_POINTER_MOTION) {
            ...
            x = libinput_event_pointer_get_absolute_x_transformed(
                pointer, grx_get_screen_width());
            y = libinput_event_pointer_get_absolute_y_transformed(
                pointer, grx_get_screen_height());
        }
    ...
    case LIBINPUT_EVENT_POINTER_BUTTON: {
        ...
        case LIBINPUT_BUTTON_STATE_PRESSED:
            ...
            if (button == ((GrxLibinputDeviceManagerSource *)source)->last_button
                && (time - ((GrxLibinputDeviceManagerSource *)source)->last_button_time)
                       < DOUBLE_PRESS_TIME) {
    ...
    case LIBINPUT_EVENT_TOUCH_CANCEL: {
        ...
        if (type == LIBINPUT_EVENT_TOUCH_DOWN || type == LIBINPUT_EVENT_TOUCH_MOTION) {
            x = libinput_event_touch_get_x_transformed(touch, grx_get_screen_width());
            y = libinput_event_touch_get_y_transformed(touch, grx_get_screen_height());
        }
        switch (type) {
        case LIBINPUT_EVENT_TOUCH_DOWN:
